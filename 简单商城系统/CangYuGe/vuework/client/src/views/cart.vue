            ...
            <div v-if="carts.length" class="cart">
                <div class="list">
                    ...
                    <ul class="body">
                        <li v-for="cart in carts" class="item" :key="cart.id">
                            <div class="check">
                                <tan-checkbox v-model="cart.checked" @change="changeChecked(cart.id, cart.checked)">&nbsp;</tan-checkbox>
                            </div>
                            <div class="img">
                                <img :src="cart.detail.image" :alt="cart.product.name + ' ' + cart.detail.name">
                            </div>
                            <div class="name">
                                <router-link :to="{ name: 'productDetail', params: {id: cart.product.id} }">
                                    <h3 :title="cart.product.name + ' ' + cart.detail.name">{{ cart.product.name + ' ' + cart.detail.name }}</h3>
                                </router-link>
                            </div>
                            <div class="price">{{ cart.detail.salePrice }} 元</div>
                            <div class="num">
                                <tan-input-number v-model="cart.quantity" :min="1" @change="changeQuantity(cart)"></tan-input-number>
                            </div>
                            <div class="total">{{ (cart.quantity * cart.detail.salePrice).toFixed(2) }} 元</div>
                            <div class="action">
                ...
                <div class="footer">
                    ...
                    <span class="total-price">
                        ...
                        <em>{{ totalPrice }}</em>
                        元
                        <tan-button primary font-size="18px" height="48px" width="200px" @click="buy">去结算</tan-button>
                    </span>
                </div>
            </div>
            <div v-else class="empty">
                <h2>您的购物车还是空的！</h2>
                <tan-button primary width="170px" height="48px" @click="$router.push({ name: 'index' })">马上去购物</tan-button>
            </div>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        changeChecked(cartId, checked) {
            cartHttp.changeChecked(checked, cartId)
                .then(res => {
                    if ( res.code != 200 ) this.$warning(res.message)
                })
        },
        changeQuantity(cart) {
            cartHttp.changeQuantity(cart.id, cart.quantity)
                .then(res => {
                    if ( res.code != 200 ) {
