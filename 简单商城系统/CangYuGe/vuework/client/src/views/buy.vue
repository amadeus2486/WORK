    ...
    <div>
        ...
        <div class="buy container">
            <div class="body">
                <div class="address">
                    ...
                    <tan-address-list :contacts="contacts" :current-id="contactCurrentId" @click="contactCurrentId = $event"></tan-address-list>
                ...
                <div class="products">
                    ...
                    <ul class="list">
                        <li v-for="cart in carts" :key="cart.id" class="item">
                            ...
                            <div class="name" :title="cart.product.name + ' ' + cart.detail.name">{{ cart.product.name + " " + cart.detail.name }}</div>
                            <div class="price-box">
                                <span class="price">{{ cart.detail.salePrice }} × {{ cart.quantity }}</span>
                                <span class="total">{{ (cart.detail.salePrice * cart.quantity).toFixed(2) }}元</span>
                            </div>
                        </li>
                    </ul>
                </div>
            ...
            <div class="footer">
                ...
                <div class="btns">
                    <tan-button width="160px" height="40px" @click="$router.go(-1)">返回购物车</tan-button>
                    <tan-button primary width="160px" height="40px" @click="buy">立即下单</tan-button>
                </div>
            </div>
        </div>
        <tan-modal 
        ...
        >
            ...
            <template v-slot:footer>
                <tan-button primary width="160px" height="40px" @click="pay">立即支付</tan-button>
                <tan-button gray width="160px" height="40px" @click="closePay">取消</tan-button>
            </template>
        </tan-modal>
    </div>
</template>
...
export default {
    ...
    computed: {
        ...
        currentContact() {
            if ( !this.contactCurrentId ) return
            for ( let tmp of this.contacts )
                if ( tmp.id == this.contactCurrentId ) return tmp
            return
        }
    },
    mounted() {
    ...
    methods: {
        ...
        buy() {
            ...
            orderHttp.buy(this.contactCurrentId, this.carts)
                .then(res => {
                    if ( res.code == 200 ) {
                        this.visible = true
