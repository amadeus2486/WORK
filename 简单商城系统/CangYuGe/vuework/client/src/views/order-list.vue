                    ...
                    <ul class="list">
                        <li v-for="detail in order.details" :key="detail.id" class="item">
                            <div v-if="detail.product">
                                ...
                                <div class="info">
                                    <router-link class="name" :to="{ name: 'productDetail', params: { id: detail.productId } }">
                                        {{ detail.product.name + " " + detail.product.details.name }}
                                    </router-link>
                                    <span class="quantity">{{ detail.product.details.salePrice + " * " + detail.quantity }}</span>
                                </div>
                            </div>
                        </li>
                    </ul>
        ...
        orderHttp.list()
            .then(res => {
                if ( res.code == 200 ) {
                    ...
                    this._orders.forEach(e => {
                        e.details.forEach(d => {
                            productHttp.getById(d.productId, d.detailId)
                                .then(res => {
                                    if ( res.code == 200 )
                                        this.$set(d, "product", res.data)
                                })
                        })
                    })
                    this.orders = this._orders
