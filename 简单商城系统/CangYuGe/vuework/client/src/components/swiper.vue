<template>
    <div class="tan-swiper-wrapper" :style="{width: width, height: height}">
        <div class="tan-swiper-image-list">
            <img v-for="image, index in images" class="tan-swiper-image-list-item" :key="image" :class="{active: index == current}" :src="image" />
        </div>
        <div class="tan-swiper-btn-list">
            <div class="tan-swiper-btn tan-swiper-btn-pre" @click="changeImage(current - 1)"></div>
            <div class="tan-swiper-btn tan-swiper-btn-next" @click="changeImage(current + 1)"></div>
        </div>
        <div class="tan-swiper-circle-list">
            <div v-for="_, index in images" :key="index" class="tan-swiper-circle-list-item" :class="{active: index == current}" @click="changeImage(index)">
                <span></span>
            </div>
        </div>
    </div>
