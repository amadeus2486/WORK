<template>
    <header class="tan-site-header">
        <div class="tan-site-topbar">
            <div class="container">
                <div class="tan-site-topbar-nav">
                    <router-link :to="{name: 'index'}">藏雨阁</router-link>
                </div>
        ...
        >
            <component :is="'tan-' + modalStatus.componentName" :data="data"></component>
            <template v-slot:footer>
                <tan-button primary width="160px" height="40px" @click="handleConfirm">{{ title }}</tan-button>
                <tan-button gray width="160px" height="40px" @click="modalVisible = false">取消</tan-button>
            </template>
        </tan-modal>
    </header>
</template>
...
export default {
    name: "TanSiteHeader",
    components: { "tan-modal": modal, "tan-login": login, "tan-register": register },
    data() {
        return {
            data: {},
            productCategory: []
